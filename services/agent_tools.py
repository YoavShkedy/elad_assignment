...
def create_search_info_tool(vector_service: VectorService):
    ...
    def search_info(
        ...
        Args:
            question (str): Natural language question from the user. The question must be fairly detailed and phrased in Hebrew.

        Returns:
            str: Relevant document excerpts from the HMO services knowledge base.
        """
        results = vector_service.search(question, k=3)
        ...
        for i, result in enumerate(results):
            formatted_results.append(f"""<context_{i+1}>\
